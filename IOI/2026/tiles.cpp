...
void init(int N, int M) {
  ...
  FOR(i, 0, n + 1) {
    FOR(j, 0, m + 1) { grid[i][j] = 0; }
  }
}

pi receive_block(int TL, int TR, int BL, int BR) {
  int32_t vals[4] = {TL, TR, BL, BR};

  int32_t st_i[4] = {n - 1, n - 1, 0, 0};
  int32_t st_j[4] = {m - 1, 0, m - 1, 0};

  int32_t end_i[4] = {0, 0, n - 1, n - 1};
  int32_t end_j[4] = {0, m - 1, 0, m - 1};

  int32_t dir_i[4] = {-1, -1, 1, 1};
  int32_t dir_j[4] = {-1, 1, -1, 1};
  ...
  for (int32_t i{st_i[type]}; i != end_i[type]; i += dir_i[type]) {
    for (int32_t j{st_j[type]}; j != end_j[type]; j += dir_j[type]) {
      if (grid[i][j] == 0 && grid[i + dir_i[type]][j] == 0 &&
          grid[i][j + dir_j[type]] == 0 &&
          grid[i + dir_i[type]][j + dir_j[type]] == 0) {
        grid[i][j] = 1;
        grid[i + dir_i[type]][j] = 1;
        grid[i][j + dir_j[type]] = 1;
        grid[i + dir_i[type]][j + dir_j[type]] = 1;

        return mp(min(i, i + dir_i[type]), min(j, j + dir_j[type]));
      }
    }
  }
  return mp(0, 0);
