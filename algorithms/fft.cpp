...
void fft(int32_t n, vector<complex<double>> &A, bool inv) {
  ...
  vector<complex<double>> A_0(n / 2, 0), A_1(n / 2, 0);
  ...
  double angle = (inv ? -1.0 : 1.0) * 2.0 * PI / n;

  complex<double> w_k = 1.0;
  complex<double> w_n = complex(cos(angle), sin(angle));

  for (int32_t i{0}; i < n / 2; i++) {
    // apply the butterfly pattern
    // for first k elements, y_k = y_k^0 + w^k_n * y_k^1
    // for second half of k elements, y_k = y_k^0 - w^k_n * y_k^1
    A[i] = A_0[i] + w_k * A_1[i];
    A[i + n / 2] = A_0[i] - w_k * A_1[i];
