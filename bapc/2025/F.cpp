...
vector<vi> get_list() {
  ...
  FOR(a, 1, 31) {
    FOR(b, 0, 31) {
      ...
      FOR(zz, 1, 31) {
        int32_t cur_point = (a * zz + b);

        cur_vals.pb(zz * 31 + (cur_point % 31));
      }

      if (sz(cur_vals) == 30) {
        all_values.pb(cur_vals);

        if (sz(all_values) == 600) {
          return all_values;
        }
      }
    }
...
void solve() {
  ...
  if (tt == "send") {
    ...
    vector<vi> list = get_list();
    ps(sz(list));

    trav(x, list[n - 1]) { pr(x, " "); }
    ps();
  } else {
    re(x, y);
