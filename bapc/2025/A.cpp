...
int32_t n;
str a;
double numbs[MAXN][MAXN];
double T[MAXN][2];
bool calced[MAXN][2];

double rec(int32_t ind, bool sign) {
  if (ind == n) {
    return 0;
  }
  ...
  FOR(i, ind, n) {
    // 0.45
    ans += 0.45 * (1.00 * (sign ? -1 : 1) * mult + rec(ind + 1, 0));

    // 0.45
    ans += 0.45 * (1.00 * (sign ? -1 : 1) * mult + rec(ind + 1, 1));

    // 0.1 continue
    cur_pow *= 0.1;
    if (i + 1 < n) {
      mult += 1.00 * (a[i + 1] - '0') * cur_pow;
    } else {
      ans += 0.1 * (sign ? -1 : 1) * mult;
      // ans += 0.1 * mult;
    }
  }
  return ans;
