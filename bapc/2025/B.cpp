...
int32_t rec(int32_t ind, int32_t prev) {
  if (ind == 16) {
    return 0;
  }

  int32_t &ans = T[ind][prev];
  if (ans != -1) {
    return ans;
  }
  ans = 1000;
  // test with top
  if (ind == 0 || check(a[prev][ind - 1], a[0][ind])) {
    ans = min(ans, rec(ind + 1, 0));
  }

  // any side
  FOR(i, 1, 5) {
    if (ind == 0 || check(a[prev][ind - 1], a[i][ind])) {
      ans = min(ans, 1 + rec(ind + 1, i));
    }
  }

  // with bottom

  if (ind == 0 || check(a[prev][ind - 1], a[5][ind])) {
    ans = min(ans, 2 + rec(ind + 1, 5));
  }
  return ans;
}
