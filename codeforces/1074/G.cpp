...
map<int32_t, int32_t> total_cnt;
int64_t needed_r[MAXN], max_mex[MAXN], orig[MAXN];

void solve() {
  total_cnt.clear();
  int64_t orig_ans = 0;
  int64_t to_mult_by = 0;
  ...
  FOR(i, 0, n) {
    ...
    to_mult_by += sz(a[i]) * (n - 1);
    orig_ans += orig_mex;
  }

  orig_ans *= (to_mult_by);

  int64_t fin_ans = orig_ans;
  ...
  FOR(i, 0, n) {
    ...
    trav(x, cc) {
      if (x.s == 1 && orig[i] > x.f) {
        // DBG((n - 1) * (orig[i] - x.f));
        fin_ans -= 1LL * (n - 1) * (orig[i] - x.f);
      }
    }

    fin_ans += (max_mex[i] - orig[i]) * total_cnt[needed_r[i]];
  }
  ps(fin_ans);
}
