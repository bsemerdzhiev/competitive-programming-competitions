...
void solve() {
  ...
  int64_t to_add = 0;
  if (n > 2) {
    to_add = (n % 2 == 1 ? n - 1 : n - 2);
  }

  int64_t biggest[2] = {INT32_MIN, INT32_MIN};
  for (int32_t i{0}; i < n; i++) {
    if (i % 2 == 0) {
      to_add = max(to_add, -2 * a[i] + i + biggest[1]);

      biggest[0] = max(biggest[0], -2 * a[i] - i);
    } else {
      // DBG(biggest[0]);
      to_add = max(to_add, 2 * a[i] + i + biggest[0]);

      biggest[1] = max(biggest[1], 2 * a[i] - i);
    }
    // DBG(i, to_add);
  }
