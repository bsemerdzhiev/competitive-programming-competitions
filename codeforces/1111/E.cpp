...
void solve() {
  int64_t lb_ans = (n - 1) * 2;
  int64_t ub_ans = n * (n - 1) / 2 + (n) / 2;

  if (k < lb_ans || k > ub_ans || k % 2 == 1) {
    ps(-1);
    return;
  }
  ...
  while (cur_node <= n) {
    int64_t cur_lb = cur_ans + 2 * rem + level[kk ^ 1];
    int64_t cur_ub = cur_lb + 2 * level[kk];

    if (k <= cur_ub && k >= cur_lb) {
      int64_t tt = cur_lb;
      FOR(i, 0, sz(sides[kk])) {
        int64_t z = sides[kk][i];

        if (k == tt) {
          ps(z, cur_node);
          FOR(j, cur_node, n) { ps(j, j + 1); }
          return;
        } else {
          tt += 2;
        }
      ...
      ps(sides[kk].back(), cur_node);
      sides[kk].push_back(cur_node++);
      level[kk]++;
      cur_ans += level[kk] + level[kk ^ 1];
