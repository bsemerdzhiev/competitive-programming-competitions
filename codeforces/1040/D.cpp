...
void update(int32_t ind, int32_t type, int32_t val) {
  for (int32_t i{ind}; i <= 2 * n; i += (i & -i)) {
    tree[type][i] += val;
  }
}

int32_t query(int32_t ind, int32_t type) {
  int32_t ans = 0;

  for (int32_t i{ind}; i > 0; i -= (i & -i)) {
    ans += tree[type][i];
  }
...
int32_t query(int32_t l, int32_t r, int32_t type) {
  return query(r, type) - query(l - 1, type);
}

void solve() {
  FOR(i, 0, 2 * n + 1) { tree[0][i] = tree[1][i] = 0; }
  FOR(i, 1, n + 1) { update(a[i], 0, 1); }

  int32_t ans = 0;
  for (int32_t i{n}; i > 0; i--) {
    update(a[i], 0, -1);

    int32_t left_inv = query(a[i], 2 * n, 0);
    int32_t right_inv = query(0, a[i], 1);

    int32_t left_inv_flipped = query(2 * n - a[i], 2 * n, 0);
    int32_t right_inv_flipped = query(0, 2 * n - a[i], 1);

    if (left_inv + right_inv < left_inv_flipped + right_inv_flipped) {
      update(a[i], 1, 1);
      ans += right_inv;
    } else {
      update(2 * n - a[i], 1, 1);
      ans += right_inv_flipped;
      ;
    }
  }
  ps(ans);
