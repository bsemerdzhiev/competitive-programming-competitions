...
void solve() {
  ...
  FOR(i, 0, n - 1) { ind.pb(i + 1); }
  FOR(i, 0, n) { possible_numbers.pb(i + 1); }

  FOR(i, 0, 30) {
    if (sz(possible_numbers) == 1) {
      break;
    }
    vi with_one, with_zero;
    vi pos_with_one, pos_with_zero;

    trav(x, possible_numbers) {
      if (x & (1 << i)) {
        pos_with_one.pb(x);
      } else {
        pos_with_zero.pb(x);
    ...
    if (sz(pos_with_one) == sz(with_one)) {
      ind = with_zero;
      possible_numbers = pos_with_zero;
    } else {
      ind = with_one;
      possible_numbers = pos_with_one;
    }
  }
  ps("!", possible_numbers[0]);
  cout.flush();
}
