...
int32_t n, k, a[MAXN];
int64_t T[365][365][365];
int64_t bst_T[365][365];

void solve() {
  set<int32_t> ss;
  vector<pair<int64_t, int64_t>> vv;

  vv.pb(mp(0, -1));
  FOR(i, 0, n) {
    if (a[i] <= k && !ss.count(a[i])) {
      ss.insert(a[i]);
      vv.push_back(mp(a[i], i));
    }
  }
  ...
  FOR(i, 1, sz(vv)) {
    ...
    FOR(j, 0, k + 1) {
      int64_t bst = -INF;
      FOR(t, 0, j + 1) {
        bst = max(bst, T[i - 1][j][t] + (vv[i].second - vv[i - 1].second) * t);
      }
      bst_T[i][j] = bst;
    }
    ...
    FOR(j, 0, k + 1) {
      FOR(z, 0, j + 1) {
        T[i][j][z] = T[i - 1][j][z] + (vv[i].second - vv[i - 1].second) * z;

        if (j - z >= 0 && z <= cur_v.first) {
          T[i][j][z] = max(bst_T[i][j - z], T[i][j][z]);
        }

        ans = max(ans, T[i][j][z] + (n - cur_v.second) * z);
      }
    }
  }
  ps(ans);
