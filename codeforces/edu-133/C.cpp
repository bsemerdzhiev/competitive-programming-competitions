...
bool check(int64_t x) {
  ...
  while (cur_pos.second < n) {
    if (a[cur_pos.first][cur_pos.second] > x) {
      // go right from prev prev_pos
      if (prev_pos.first == cur_pos.first) {
        mark[prev_pos.first][prev_pos.second] = 0;

        prev_pos.first ^= 1;
        x--;
      }
      prev_pos.second++;
      while (prev_pos.second < n) {
        if (a[prev_pos.first][prev_pos.second] > x)
          return false;
        prev_pos.second++;
        x++;
      }
      ...
      while (prev_pos.second >= 0 && !mark[prev_pos.first][prev_pos.second]) {
        if (a[prev_pos.first][prev_pos.second] > x)
          return false;
        x++;
