...
int32_t n, l[MAXN], r[MAXN], u[MAXN], v[MAXN];

bool check(int32_t cc) {
  int32_t max_made = 0;
  for (int32_t j = 1; j <= n; j++) {
    int32_t rem = cc - max_made;
    // ps(j, max_made);
    if ((l[j] > max_made + 1 || max_made + 1 > r[j]) &&
        (rem < u[j] || rem > v[j])) {
      max_made++;
    }
  }
...
int main() {
  ...
  while (t--) {
    re(n);
    FOR(i, 1, n + 1) { re(l[i], r[i], u[i], v[i]); }
