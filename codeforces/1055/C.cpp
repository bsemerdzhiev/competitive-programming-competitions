...
void solve() {
  vi cnt[2];
  vi cnt_more[2];
  cnt[0] = vector<int32_t>(n + 1, 0);
  cnt[1] = vector<int32_t>(n + 1, 0);

  cnt_more[0] = vector<int32_t>(n + 1, 0);
  cnt_more[1] = vector<int32_t>(n + 1, 0);

  a[0] = -1;
  FOR(i, 1, n + 1) {
    FOR(j, 0, 2) { cnt[j][i] = cnt[j][i - 1] + (a[i] == j); }
    FOR(j, 0, 2) { cnt_more[j][i] = cnt_more[j][i - 1] + (a[i] == a[i - 1]); }
  }
  int32_t l, r;
  FOR(i, 0, q) {
    re(l, r);

    int32_t br_ones = cnt[1][r] - cnt[1][l - 1];
    int32_t br_zeroes = cnt[0][r] - cnt[0][l - 1];

    if (br_ones % 3 != 0 || br_zeroes % 3 != 0) {
      ps(-1);
      continue;
    }

    int32_t br[2] = {cnt_more[1][r] - cnt_more[1][l],
                     cnt_more[0][r] - cnt_more[0][l]};
