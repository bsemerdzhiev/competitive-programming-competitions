...
int32_t n, q;
int32_t l[MAXN], r[MAXN];

int32_t query(int32_t v, int32_t w) {
  ps(std::format("? {} {}", v, w));
...
bool check(int32_t x) {
  ...
  for (int32_t i{30}; i >= 0; i--) {
    int32_t update_l = cur_l + (1 << i);

    if (update_l <= n && query(update_l, cur_r) >= x) {
      cur_l = update_l;
    }
  }
  for (int32_t i{30}; i >= 0; i--) {
    int32_t update_r = cur_r - (1 << i);
    if (update_r >= cur_l && query(cur_l, update_r) >= x) {
      cur_r = update_r;
    }
  }
  FOR(i, 0, q) {
    if (l[i] <= cur_l && r[i] >= cur_r) {
      return true;
    }
  }
  return false;
