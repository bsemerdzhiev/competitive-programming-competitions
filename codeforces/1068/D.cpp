...
int64_t rec(int32_t ind, int32_t needed_after) {
  if (ind == sz(s)) {
    return k - needed_after;
  }
  if (T[ind][needed_after] != -1) {
    return T[ind][needed_after];
  }

  int32_t needed = 0;
  int64_t ans = 0;
  // DBG(ind, sz(s));
  for (int32_t j{ind}; j < sz(s); j++) {
    if (s[j] == '0') {
      needed++;
    }
    // DBG(ind, needed);
    if (needed_after + needed + 1 <= k) {
      // DBG(needed);
      ans = max(ans, rec(j + 1, needed + needed_after + 1) + (j - ind + 1));
    } else {
      break;
    }
  }
  ans = max(ans, rec(ind + 1, needed_after));
  return T[ind][needed_after] = ans;
}
