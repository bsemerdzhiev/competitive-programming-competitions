...
int32_t n, a[MAXN];
int32_t T[MAXN][MAXN][MAXN];

int32_t rec(int32_t ind, int32_t up, int32_t up2) {
  if (up > up2) {
    swap(up, up2);
  }
  if (ind == n) {
  ...
  if (a[ind] >= min(up, up2)) {
    if (up2 <= a[ind]) {
      ans = MO::add(ans, rec(ind + 1, up, a[ind]));
    } else {
      ans = MO::add(ans, rec(ind + 1, a[ind], up2));
    }
  }
  ans = MO::add(ans, rec(ind + 1, up, up2));
