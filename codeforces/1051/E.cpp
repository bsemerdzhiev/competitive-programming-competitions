...
int32_t n, a[MAXN];
int32_t tree[MAXN][MAXN];

void update(int32_t row, int32_t v, int32_t val) {
  for (int32_t i{v + 1}; i <= n + 1; i += (i & -i)) {
    tree[row][i] = MO::add(tree[row][i], val);
  }
}

int32_t query(int32_t row, int32_t v) {
  int32_t ans = 0;
  for (int32_t i{v + 1}; i > 0; i -= (i & -i)) {
    ans = MO::add(ans, tree[row][i]);
  }
  return ans;
}

int32_t query(int32_t row, int32_t i, int32_t j) {
  return MO::add(query(row, j), -query(row, i - 1));
}

void solve() {
  FOR(i, 0, n + 2) {
    FOR(j, 0, n + 2) { tree[i][j] = 0; }
  }
  ...
  FOR(i, 1, n + 1) {
    FOR(j, 0, n + 1) {
      if (a[i] >= j) {
        int32_t cur_ans = query(j, j, a[i]);

        update(a[i], j, cur_ans);
        update(j, a[i], cur_ans);
        ans = MO::add(ans, cur_ans);
      } else {
        int32_t cur_ans = query(j, 0, a[i]);
