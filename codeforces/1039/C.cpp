...
bool check_if_any_smaller(int32_t x) {
...
void solve() {
  ...
  // ps(check_if_any_smaller(2));

  for (int32_t i{n - 1}; i >= 0; i--) {
    bool c1 = check_if_any_smaller(a[i]);

    if (c1) {
      int32_t first_h = (a[i] + 1) / 2;
      int32_t second_h = a[i] - first_h;
      if (first_h == second_h) {
        first_h++;
        second_h--;
      }
      ...
      if (check_if_any_smaller(first_h)) {
        ps("NO");
        return;
      }
    }
