...
int32_t rec(int32_t ind, int32_t prev_sum, int32_t prev_num) {
  if (ind == n) {
    return 1;
  }
  int32_t &ans = T[ind][prev_sum][prev_num];
  ...
  if (a[ind] == '?') {
    FOR(i, 0, 2) {
      int32_t cur_sum = prev_num + i;
      if (cur_sum != prev_sum) {
        ans = MO::add(ans, rec(ind + 1, ind == 0 ? 3 : cur_sum, i));
      }
    }
  } else {
    int32_t cur_sum = prev_num + (a[ind] - '0');
    if (cur_sum != prev_sum) {
      ans = rec(ind + 1, ind == 0 ? 3 : cur_sum, a[ind] - '0');
    }
  }
  return ans;
}
