...
void solve() {
  ...
  if (cmd == "first") {
    ...
    sums[0] = (sums[0] + 100 * n - br * (x - 1)) % n;
    sums[1] = (sums[1] + 100 * n - br * (y - 1)) % n;

    pi ans_cell;
    trav(some_cell, can_take) {
      int32_t prev_s0 = sums[0];
      int32_t prev_s1 = sums[1];

      sums[0] = (-sums[0] + 10 * n + some_cell.first) % n;
      sums[1] = (-sums[1] + 10 * n + some_cell.second) % n;

      if (ss.count(mp(sums[0], sums[1]))) {
        sums[0] = prev_s0;
        sums[1] = prev_s1;
      } else {
        ans_cell = some_cell;
    ...
    ps(ans_cell.first + 1, ans_cell.second + 1, sums[0] + 1, sums[1] + 1);
  } else {
    sums[0] %= n;
    sums[1] %= n;
    ...
    while (sums[0] != 0) {
      ...
      sums[0] = (100 * n + sums[0] - br) % n;
    }

    while (sums[1] != 0) {
      ans_y = (n + ans_y + 1) % n;
      sums[1] = (100 * n + sums[1] - br) % n;
    }
