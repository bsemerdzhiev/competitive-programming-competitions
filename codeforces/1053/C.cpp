...
void solve() {
  vector<int64_t> pref_sum(2 * n + 2, 0);
  vector<int64_t> alt_sum(2 * n + 2, 0);

  FOR(i, 1, 2 * n + 1) {
    pref_sum[i] = pref_sum[i - 1] + a[i];
    alt_sum[i] = alt_sum[i - 1] + (i % 2 == 1 ? -a[i] : a[i]);
    // ps(alt_sum[i], i);
  }
  pref_sum[2 * n + 1] = pref_sum[2 * n];
  alt_sum[2 * n + 1] = alt_sum[2 * n];

  int32_t k = 2 * n + 1;
  FOR(i, 1, n + 1) {
    int64_t sum = (pref_sum[k] - pref_sum[k - i]) - pref_sum[i - 1];

    int64_t to_add = (alt_sum[k - i] - alt_sum[i - 1]);
    // ps(to_add);
