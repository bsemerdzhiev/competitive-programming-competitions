...
int64_t rec(int32_t ind) {
  ...
  cur_ans = abs(a[ind] - a[ind + 1]) + rec(ind + 2);
  ...
  if (n - ind >= 3) {
    ...
    cur_ans = min(cur_ans, rec(ind + 3) + vv[2] - vv[0]);
  }
  return T[ind] = cur_ans;
}

void solve() {
  // leave a at 0
  int64_t best_ans = 1e15;
  FOR(z, 0, 5) {
    FOR(i, 0, n) { T[i] = -1; }
    best_ans = min(best_ans, rec(0));
