...
int32_t n;
str a[MAXN];
bool was_marked[MAXN][MAXN][2][5];
int32_t cnt = 0;

bool mark(int32_t i, int32_t j, int32_t dir) {
  int32_t d_i = 1;
  int32_t d_j = (dir == 0 || dir == 2) ? -1 : 1;
  int32_t st = dir >= 2 ? 1 : 0;
  ...
  while (i < n && j >= 0 && j < n) {
    if (was_marked[i][j][st][dir])
      return false;
    was_marked[i][j][st][dir] = 1;
    if (a[i][j] == '#')
      cur_cnt--;
...
void solve() {
  FOR(i, 0, n) {
    FOR(j, 0, n) {
      FOR(z, 0, 2)
      FOR(g, 0, 5) { was_marked[i][j][z][g] = 0; }
    }
  }
  cnt = 0;
  FOR(i, 0, n) {
  ...
  FOR(i, 0, n) {
    FOR(j, 0, n) {
      if (mark(i, j, 0) || mark(i, j, 1) || mark(i, j, 2) || mark(i, j, 3) ||
          mark(i, j, 4)) {
        ps("YES");
        return;
      }
