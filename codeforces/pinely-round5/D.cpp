...
void solve() {
  vpi vv;
  vv.push_back(mp(-1, 0));
  FOR(i, 1, n + 1) { vv.pb(mp(a[i], i)); }
  sort(vv.begin(), vv.end(), [](const pi &f, const pi &s) {
  ...
  trav(x, vv) {
    ...
    while (l <= r) {
      ...
      if (vv[middle].first + 1 < x.first ||
          (vv[middle].first + 1 == x.first && vv[middle].second > x.second)) {
        l = middle + 1;
        ans = middle;
      } else {
        r = middle - 1;
    ...
    T[k] = max(pref[ans], best[x.first]) + 1;
    pref[k] = max(pref[k - 1], T[k]);
    best[x.first] = max(best[x.first], T[k]);
    k++;
  }
  ps(n - pref[n]);
}
