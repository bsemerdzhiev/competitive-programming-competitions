...
void solve() {
  int64_t n = r - l + 1;
  vector<int64_t> ans_v(n, 0);
  FOR(i, 0, n) { ans_v[i] = l + i; }
  vector<bool> marked(n, 0);

  for (int32_t i{30}; i >= 0; i--) {
    for (int32_t j{1}; j < n; j++) {
      if ((ans_v[j] ^ ans_v[j - 1]) & (1 << i) && !marked[j] &&
          !marked[j - 1]) {
        int32_t st_up = j, st_dwn = j - 1;

        while (st_up < n && st_dwn >= 0) {
          if (((ans_v[st_up] ^ ans_v[st_dwn]) & (1 << i)) == 0 ||
              marked[st_up] || marked[st_dwn]) {
            break;
          }

          marked[st_up] = marked[st_dwn] = 1;
          swap(ans_v[st_up], ans_v[st_dwn]);
