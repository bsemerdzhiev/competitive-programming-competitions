...
void solve() {
  vector<int64_t> add_ans(n + 1, 0);

  int64_t fin_ans = 0;
  FOR(i, 1, n + 1) {
    int64_t max_dirt = a[i];
    int64_t cur_ans = 0;

    for (int32_t j{i}; j >= 1; j--) {
      max_dirt = max(max_dirt, a[j]);
      cur_ans += h - max_dirt;
    }
  ...
  FOR(i, 1, n + 1) {
    int64_t max_tile = i + 1;
    fin_ans = max(fin_ans, add_ans[i]);
    FOR(j, i + 2, n + 1) {
      if (a[max_tile] < a[j - 1]) {
        max_tile = j - 1;
      }
      fin_ans = max(fin_ans, add_ans[i] + add_ans[j] - add_ans[max_tile]);
    }
  }
