...
void solve() {
  FOR(i, 1, n + 1) {
    FOR(j, 1, n + 1) { asked[i][j] = 0; }
  }

  vi cnt(n + 1, 0);
  for (;;) {
    int32_t cnt_best = 2 * n;
    pi chosen_p;

    FOR(i, 1, n + 1) {
      FOR(j, i + 1, n + 1) {
        if (!asked[i][j] && cnt[i] + cnt[j] < cnt_best) {
          chosen_p = mp(i, j);
          cnt_best = cnt[i] + cnt[j];
        }
      }
    }

    ps(chosen_p.f, chosen_p.s);
    cout.flush();
    int32_t res;
    re(res);
    ...
    asked[chosen_p.f][chosen_p.s] = asked[chosen_p.s][chosen_p.f] = 1;
    cnt[chosen_p.f]++;
    cnt[chosen_p.s]++;
  }
}
