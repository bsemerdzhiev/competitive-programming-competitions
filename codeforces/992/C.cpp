...
void solve() {
  int64_t rem_numbers = n;
  set<int32_t> ss;
  FOR(i, 1, n + 1) { ss.ins(i); }
  int64_t rem = k;
  ...
  while (true) {
    if (rem_numbers >= 45) {
      ...
      vi direcitons;
      int64_t rem_symbols = rem_numbers - 1;

      while (rem_symbols) {
        // DBG((1 << (rem_numbers - 1)));
        if ((1LL << (rem_symbols - 1)) < rem) {
          direcitons.push_back(1);
          rem -= (1LL << (rem_symbols - 1));
        } else {
          direcitons.pb(0);
        }
        rem_symbols--;
      ...
      deque<int32_t> dd;
      dd.push_back(*ss.rbegin());
      ss.erase(*ss.rbegin());

      for (int32_t i{sz(direcitons) - 1}; i >= 0; i--) {
        // ps(direcitons[i]);
        if (direcitons[i] == 0) {
          dd.push_front(*ss.rbegin());
          ss.erase(*ss.rbegin());
        } else {
          dd.push_back(*ss.rbegin());
          ss.erase(*ss.rbegin());
