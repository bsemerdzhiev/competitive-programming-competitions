...
void solve() {
  ...
  FOR(i, 0, n) {
    FOR(z, 0, 2) {
      trav(x, primes[a[i].second + z]) {
        ans = min(bst[x] + int64_t(z) * a[i].first, ans);

        bst[x] = min(bst[x], int64_t(z) * a[i].first);
      }
    }
  }
  ...
  FOR(i, 1, n) {
    FOR(z, 0, 1) {
      trav(x, primes[a[i].s + z]) {
        bst[x] = min(bst[x], int64_t(z) * a[i].first);
      }
    }
  }

  int32_t wanted_el = a[0].second;
  trav(x, only_primes) {
    if (bst[x] == INF)
      continue;
    if (x < wanted_el) {
      int64_t to_add = (x * ((wanted_el + x - 1) / x)) - wanted_el;
      ans = min(ans, to_add * a[0].first);
    } else {
      ans = min(ans, int64_t(x - wanted_el) * a[0].first);
    }
  }
...
int main() {
  ...
  while (t--) {
    re(n);
    FOR(i, 0, n) { re(a[i].second); }
    FOR(i, 0, n) { re(a[i].first); }
