...
int64_t query(int32_t l, int32_t r) { return pref[r] - pref[l - 1]; }

void solve() {
  sort(a + 1, a + n + 1);

  auto calc_for = [](int32_t ind, int32_t ss) {
    int64_t will_take = min(int64_t(ind - 1), int64_t(ss * k));

    int64_t from_left = will_take * a[ind] - pref[will_take];

    int64_t from_right =
        ((ss + 1) * a[ind]) - query(ind, min(int64_t(ind + ss), n));

    return from_left + from_right + pref[n];
  };
  ...
  int64_t best_ans = 0;
  FOR(i, 1, n + 1) {
    int32_t l = 1, r = min(int32_t(n - i), i - 1);
    int32_t ans = 0;

    while (l <= r) {
      int32_t middle = (l + r) >> 1;

      if (calc_for(i, middle) >= calc_for(i, middle - 1)) {
        l = middle + 1;
        ans = middle;
      } else {
        r = middle - 1;
    ...
    best_ans = max(best_ans, calc_for(i, ans));
  }
  ps(best_ans);
}
