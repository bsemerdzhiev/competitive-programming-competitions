...
void rec(int32_t v, int32_t p, int32_t lvl, vpi &moves, int32_t &had_moved) {
  trav(x, graph[v]) {
    if (x == p)
      continue;
    rec(x, v, lvl ^ 1, moves, had_moved);
  }
  if (v == n)
    return;
  if (had_moved == lvl) {
...
int32_t pre_rec(int32_t v, int32_t p, int32_t vv) {
  if (v == n)
    return vv;
...
void solve() {
  vpi moves;
  int32_t had_moved = 0;
  int32_t start_lvl = pre_rec(1, 1, 0);

  rec(n, n, start_lvl, moves, had_moved);
