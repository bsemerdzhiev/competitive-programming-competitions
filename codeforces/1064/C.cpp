...
void solve() {
  ...
  FOR(i, 0, q) {
    ...
    int32_t rem_number = c;
    int32_t ans = 0;
    priority_queue<int32_t> ss;
    FOR(j, 0, min(60, n)) { ss.push(a[j]); }

    while (rem_number > 0) {
      int32_t cur_one = __lg(rem_number);

      int32_t cur_number = ss.top();
      ss.pop();

      if (cur_number >= (1 << cur_one)) {
        cur_number -= (1 << cur_one);
        rem_number ^= (1 << cur_one);

        ss.push(cur_number);
      } else {
        ans += (1 << cur_one) - cur_number;
