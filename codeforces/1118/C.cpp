...
bool query(int32_t u, int32_t v, int32_t d) {
  ps("?", u, v, d);
  cout.flush();
  int32_t ans;
  re(ans);
...
void report(int32_t u, int32_t v, int32_t d) {
  ps("!", u, v, d);
  cout.flush();
}
...
void solve() {
  ...
  FOR(i, 1, n + 1) {
    while (cur_d < n && i != new_node && query(new_node, i, cur_d)) {
      u = i;
      cur_d++;
    }
  }
  report(u, new_node, cur_d - 1);
}
