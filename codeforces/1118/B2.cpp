...
void solve() {
  FOR(i, 0, 20) { answer[i] = 0; }
  vl br(m + 1, 0);

  FOR(i, 0, n) { br[a[i]]++; }

  FOR(i, 1, m + 1) { br[i] += br[i - 1]; }

  for (int64_t current_k{0}; current_k < 20; current_k++) {
    for (int64_t x{1}; x <= m; x++) {
      int64_t cur_ans = 0;

      for (int64_t y{1}; y <= m; y++) {
        int64_t start_from = x * y;
        if (start_from > m)
          break;
        int64_t up_to = x * y + x - 1;

        int64_t cnt = br[min(m, up_to)] - br[start_from - 1];

        cur_ans += cnt * min(int64_t((1LL << (current_k + 1)) - 1LL), y);

        if (y == (1LL << (current_k + 1))) {
          cur_ans += br[start_from] - br[start_from - 1];
        }
      }
      answer[current_k] = max(answer[current_k], cur_ans);
    }
  }
  ...
  FOR(i, 0, m) {
    if (i < 20) {
      best_ans = max(best_ans, answer[i]);
    }
    pr(best_ans, " ");
  }
  ps();
