...
void solve() {
  memset(T, 0, sizeof(T));
  FOR(i, 0, n + 1) {
    FOR(j, 0, n * (n - 1) / 2 + 1) { T[i][j] = 0; }
  }

  T[0][0] = 1;
  for (int32_t i{0}; i <= n; i++) {
    for (int32_t j{0}; j <= n * (n - 1) / 2; j++) {
      for (int32_t z{1}; z <= n; z++) {

        int32_t prev_block = j - ((z) * (z - 1) / 2);

        if (prev_block >= 0 && i - z >= 0) {
          T[i][j] = max(T[i][j], T[i - z][prev_block]);
        }
      }
    }
  }
  if (T[n][(n * (n - 1) / 2) - k]) {
    int32_t cur_segm = (n) * (n - 1) / 2 - k;
    int32_t rem_n = n;

    while (rem_n > 0) {
      int32_t chosen_int = 0;

      for (int32_t i{1}; i <= rem_n; i++) {
        if (cur_segm - (i) * (i - 1) / 2 >= 0 &&
            T[rem_n - i][cur_segm - (i - 1) * i / 2]) {
          chosen_int = i;
          break;
        }
      }

      FOR(i, 0, chosen_int) { pr(rem_n - (chosen_int - i - 1), " "); }

      rem_n -= chosen_int;
      cur_segm -= (chosen_int - 1) * chosen_int / 2;
    }
    ps();
