...
void solve() {
  ...
  FOR(i, 0, m) {
    ...
    int64_t new_val_one = cur_val - d;
    int64_t new_val_two = cur_val;

    int64_t new_sum_one = 0;
    int64_t new_sum_two = cur_sum + 1;

    for (int32_t j{0}; j < m; j++) {
      int64_t to_add = (j == 0 ? p[j] : p[j] - p[j - 1]);
      ...
      while (prev_ind + 1 < 2 * m && p[prev_ind + 1] <= new_sum_two) {
        new_val_two += r[prev_ind + 1];

        prev_ind++;
      }
      if (new_val_one > new_val_two) {
        ps("YES");
        return;
      }
    }
