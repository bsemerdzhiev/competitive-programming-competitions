...
void solve() {
  ...
  vi sums[4];
  FOR(i, 0, 4) { sums[i] = vector<int32_t>(n + 1, 0); }

  FOR(i, 1, n + 1) {
    FOR(j, 0, 4) { sums[j][i] = sums[j][i - 1]; }
    if (a[i] == '0' && b[i] == '0') {
      sums[0][i]++;
    } else if (a[i] == '0' && b[i] == '1') {
      sums[1][i]++;
  ...
  FOR(i, 0, q) {
    ...
    int32_t types[4] = {0, 0, 0, 0};
    FOR(j, 0, 4) { types[j] = sums[j][r] - sums[j][l - 1]; }

    int32_t mm = min(types[1], types[2]);
    types[1] -= mm;
    types[2] -= mm;

    int32_t rem = max(types[1], types[2]);

    if (types[0] + types[3] < rem) {
      ps("NO");
    } else {
      ps("YES");
    }
