...
void solve() {
  ...
  while (true) {
    ...
    while (true) {
      ...
      int32_t sm = MAXN * MAXN;

      for (int32_t j{0}; j < k; j++) {
        int32_t row_ind = av[j];

        while (true) {
          int32_t &x = vv[row_ind];
          if (x < 0) {
            new_k = 0;
            break;
          }

          int32_t element = a[row_ind][x];
          if (vis.count(element)) {
            x--;
            continue;
          }
      ...
      if (k == 1) {
        int32_t row = av[0];
        for (int32_t j{vv[row]}; j >= 0; j--) {
          if (!vis.count(a[row][j])) {
            vis.insert(a[row][j]);
            pr(a[row][j], " ");
          }
        }
