...
void solve() {
  ...
  FOR(z, 0, k) {
    ...
    int32_t g = 0;
    map<int32_t, stack<int32_t>> pos;

    trav(x, cur_a) { pos[x].push(g++); }
    vi will_need(n, 0);

    int32_t left_most = n;
    for (int32_t i{n - 1}; i >= 0; i--) {
      if (cur_a[i] != b[i]) {
        while (!pos[b[i]].empty() && pos[b[i]].top() > min(left_most, i)) {
          pos[b[i]].pop();
        }
        // if (i == 1) {
        //   ps(left_most);
        ...
        will_need[pos[b[i]].top()] = 1;
        left_most = min(left_most, pos[b[i]].top());
      } else if (i > left_most) {
        while (!pos[b[i]].empty() && pos[b[i]].top() > min(left_most, i - 1)) {
          pos[b[i]].pop();
        }
        if (pos[b[i]].empty()) {
          ps(-1);
