...
int64_t rec(int32_t ind, int32_t type) {
  if (ind == n) {
    return 0;
  }

  int64_t &ans = T[ind][type];
  if (ans != -1) {
    return ans;
  }
  ans = 0;
  ...
  if (type == 0) {
    ans = a[ind] + rec(ind + 1, 1);
  } else if (type == 1) {
    ans = (a[ind] - 1) + rec(ind + 1, 1);

    ans = min(ans, a[ind] + rec(ind + 1, 2));
  } else {
    ans = a[ind] + rec(ind + 1, 2);

    ans = min(ans, max(0, a[ind] - ind) + rec(ind + 1, 1));
  }
  return ans;
}
