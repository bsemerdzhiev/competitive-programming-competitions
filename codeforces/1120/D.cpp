...
int64_t n, a[MAXN], p[MAXN];
int64_t tree[MAXN];

void update(int32_t ind, int64_t val) {
  for (int32_t i{ind}; i <= n; i += (i & -i)) {
    tree[i] += val;
  }
}

int64_t query(int32_t ind) {
  int64_t ans = 0;
...
int64_t query(int32_t l, int32_t r) { return query(r) - query(l - 1); }
...
void solve() {
  ...
  for (int64_t i{n}; i >= 1; i--) {
    auto it = ss.insert(p[i]).first;
    set<int32_t>::iterator next_it = next(it);
    ...
    if (it != ss.begin()) {
      ...
      int64_t sum = query(prev_ind, p[i] - 1);
    ...
    while (next_it != ss.end()) {
      int64_t cur_sum = query(prev_ind, *next_it - 1);

      if (cur_sum < a[*next_it]) {
        break;
      }
      next_it = ss.erase(next_it);
    }
    answer[i - 1] = sz(ss) - 1;
  }
  FOR(i, 0, n) { pr(answer[i], " "); }
  ps();
  FOR(i, 0, n + 1) { tree[i] = 0; }
}
