...
void solve() {
  ...
  priority_queue<int32_t> save_pq[2];
  save_pq[0] = pq[0];
  save_pq[1] = pq[1];
  ...
  for (int32_t i{0}; i < 2; i++) {
    ...
    int32_t current_ans = 0;
    while (true) {
      if (pq[0].top() <= 1 && pq[1].top() <= 1) {
        break;
      }
      if (pq[current].top() >= 1) {
        int32_t to_add = pq[current].top();
        if (to_add == 1 && sz(pq[current]) < sz(pq[current ^ 1])) {
          current_ans = 2 * n;
          break;
        }
        pq[current].pop();
        if (to_add - 1 > 0) {
          pq[current].push(to_add - 1);
        }
        current_ans++;
