...
int64_t n, x[MAXN], y[MAXN];

int64_t jmp(char dir, int64_t k) {
  ps("?", dir, k);
  cout.flush();
  int64_t resp;
  re(resp);
...
int32_t get_closest(int64_t X, int64_t Y) {
  int64_t sm = INT64_MAX;
  int32_t to_ret;
  FOR(i, 0, n) {
    if (abs(X - x[i]) + abs(Y - y[i]) < sm) {
      sm = abs(X - x[i]) + abs(Y - y[i]);
      to_ret = i;
    }
  }
  return to_ret;
...
void solve() {
  int32_t left_low = get_closest(-2e9, -2e9);
  int32_t right_low = get_closest(2e9, -2e9);
  ...
  int64_t s2 = jmp('R', int64_t(1e9));

  int64_t delta_y = (s1 + s2 - int64_t(4e9) + (x[right_low] - x[left_low]) -
                     (y[right_low] - y[left_low])) /
                    2LL;

  int64_t real_y = y[left_low] - delta_y;

  int64_t real_x = x[left_low] - (s1 - delta_y);

  ps("!", real_x + int64_t(2e9), real_y + int64_t(2e9));
  cout.flush();
}
