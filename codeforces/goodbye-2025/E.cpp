...
int64_t query(int32_t l, int32_t r) {
  ps(std::format("? {} {}", l, r));
  cout.flush();
  int64_t ans;
  re(ans);
...
void report_answer(int64_t x) {
  ps(std::format("! {}", x));
  cout.flush();
}

void rec(int32_t st, int32_t end, int64_t total_sum) {
  if (st == end) {
    report_answer(query(st, end));
    return;
  }
  // ps(st, end);
  int32_t l = 1, r = end - st + 1;

  while (l <= r) {
    int32_t middle = (l + r) >> 1;
    int64_t cur_sum = query(st, st + middle - 1);

    if (total_sum - cur_sum == cur_sum) {
      // ps(st, middle, end - st + 1 - middle, middle);
      if (end - st + 1 - middle > middle) {
        rec(st, st + middle - 1, total_sum - cur_sum);
      } else {
        rec(st + middle, end, total_sum - cur_sum);
      }

      return;
    } else if (total_sum - cur_sum > cur_sum) {
      l = middle + 1;
    } else {
      r = middle - 1;
    }
