...
void solve() {
  ...
  int32_t diag_node = 1;
  FOR(i, 1, n + 1) {
    if (depth[i] > depth[diag_node]) {
      diag_node = i;
    }
  }
  depth[diag_node] = 0;
  dfs(diag_node, diag_node);

  int32_t diag_two = 1;
  FOR(i, 1, n + 1) {
    if (depth[i] > depth[diag_two]) {
      diag_two = i;
    }
  }
  ...
  while (diag_two != diag_node) {
    all_diag_nodes.pb(diag_two);

    diag_two = par[diag_two];
  }
  all_diag_nodes.pb(diag_node);

  int32_t k = sz(all_diag_nodes);

  for (int32_t i{1}; i < k - 1; i++) {
    if (graph[all_diag_nodes[i]].size() > 2) {
      set<int32_t> ss;
      ss.insert(all_diag_nodes[i - 1]);
      ss.insert(all_diag_nodes[i + 1]);
      int32_t ch;

      trav(x, graph[all_diag_nodes[i]]) {
        if (!ss.count(x)) {
          ch = x;
          break;
        }
      ...
      ps(all_diag_nodes[i - 1], all_diag_nodes[i], ch);
      return;
    }
  }
