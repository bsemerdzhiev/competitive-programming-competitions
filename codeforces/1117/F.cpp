...
void init(int32_t n) {
  ...
  FOR(i, 0, N) {
    FOR(j, 0, 5) {
      FOR(z, 0, 5) { tree[i].val[j][z] = INF; }
    }
  }
}

node_type merge_nodes(const node_type &l, const node_type &r) {
  if (l.val[0][0] == -1) {
    return r;
  } else if (r.val[0][0] == -1) {
    return l;
  ...
  while (d > 0 && l.val[d - 1][0] == INF) {
    d--;
  }
  FOR(i, 0, x - d) {
    FOR(j, 0, x) { res.val[d + i][j] = r.val[i][j]; }
  }

  for (int32_t left_left{0}; left_left < d; left_left++) {
    for (int32_t right_right{0}; right_right < x; right_right++) {
      res.val[left_left][right_right] = INF;
      for (int32_t middle{0}; middle < x; middle++) {
        if (r.val[middle][right_right] == INF) {
          break;
        }
        res.val[left_left][right_right] =
            min(res.val[left_left][right_right],
                l.val[left_left][middle] + r.val[middle][right_right]);
      }
    }
  }
  return res;
...
void update_node(int32_t k, int32_t len, int32_t pen) {
  FOR(i, 0, min(len, x)) {
    FOR(j, 0, x) {
      int64_t len_to_jump = len + j - i;

      tree[k].val[i][j] = ((len_to_jump + x - 1) / x) * pen;
...
node_type query(int32_t k, int32_t l, int32_t r, int32_t i, int32_t j) {
  if (r <= j && l >= i) {
    return tree[k];
  }
  int32_t middle = (l + r) >> 1;
  ...
  if (j <= middle) {
    ...
    return query(k << 1 | 1, middle + 1, r, i, j);
  } else {
    return merge_nodes(query(k << 1, l, middle, i, j),
