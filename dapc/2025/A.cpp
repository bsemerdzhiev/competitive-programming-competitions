...
int32_t n, l, r, a[MAXN];

pi min_and_max[MAXN][MAXN];

bool check(int32_t lft, int32_t rght) {
  int32_t top_most = min_and_max[lft][rght].s;
  int32_t bottom_most = min_and_max[lft][rght].f;

  if (top_most > r && bottom_most < l) {
    return false;
  } else if (top_most > r) {
    int32_t k = (top_most - r + 11) / 12;

    return bottom_most - 12 * k >= l;
  } else if (bottom_most < l) {
    // DBG("Yes");
    int32_t k = (l - bottom_most + 11) / 12;

    return top_most + 12 * k <= r;
  } else {
    return true;
  }
}
...
void solve() {
  FOR(i, 0, n) {
    int32_t mm = a[i], mx = a[i];
    for (int32_t j{i}; j >= 0; j--) {
      mm = min(mm, a[j]);
      mx = max(mx, a[j]);

      min_and_max[j][i] = {mm, mx};
    }
  }
  vi T(n, 0);
  ...
  for (int32_t i{0}; i < n; i++) {
    for (int32_t j{i}; j >= 0; j--) {
      if (check(j, i)) {
        T[i] = max(T[i], min(j == 0 ? n : T[j - 1], (i - j + 1)));
      }
    }
  }
  ps(T[n - 1]);
