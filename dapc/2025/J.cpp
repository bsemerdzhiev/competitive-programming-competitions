...
void solve() {
  vector<int32_t> indeces(n, 0);
  FOR(i, 0, n) { sort(v[i].begin(), v[i].end(), greater<string>()); }

  for (int32_t i{n - 2}; i >= 0; i--) {
    while (indeces[i] < v[i].size() &&
           v[i][indeces[i]] > v[i + 1][indeces[i + 1]]) {
      // DBG(i, v[i].size());
      indeces[i]++;
    }
    if (indeces[i] >= v[i].size()) {
      ps("impossible");
      return;
    }
  }
  FOR(i, 0, n) { ps(v[i][indeces[i]]); }
}
